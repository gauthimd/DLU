...
class System():
  ...
  def turnon(self, color): # color must be a color primary key or string hexcode "#AA33FF"
      ...
      for x in t: 
          pwm.set_pwm(x.redpin, 0, int(ncolor.redpwm*self.bright))
          pwm.set_pwm(x.greenpin, 0, int(ncolor.greenpwm*self.bright))
          pwm.set_pwm(x.bluepin, 0, int(ncolor.bluepwm*self.bright))
  ...
  def turnon3separate(self, colors): # colors must be list of color primary keys or string hexcodes "#AA33FF"
      ...
      y = 0
      for x in t:
          pwm.set_pwm(x.redpin, 0, int(ncolors[y].redpwm*self.bright))
          pwm.set_pwm(x.greenpin, 0, int(ncolors[y].greenpwm*self.bright))
          pwm.set_pwm(x.bluepin, 0, int(ncolors[y].bluepwm*self.bright))
  ...
  def shift3separate(self, colors): # colors must be list of color primary keys or string hexcodes "#AA33FF"
    ...
    for i in range(int(maxdiff/100)):
      for x in t:
        if 100*i <= x.reddiff:
          if x.redpwm < ncolors[y].redpwm*self.bright:
            pwm.set_pwm(x.redpin, 0, x.redpwm + 100*i)
          elif x.redpwm > ncolors[y].redpwm*self.bright:
            pwm.set_pwm(x.redpin, 0, x.redpwm - 100*i)
        else:
          pwm.set_pwm(x.redpin, 0, int(ncolors[y].redpwm*self.bright))
        if 100*i <= x.greendiff:
          if x.greenpwm < ncolors[y].greenpwm*self.bright:
            pwm.set_pwm(x.greenpin, 0, x.greenpwm + 100*i)
          elif x.greenpwm > ncolors[y].greenpwm*self.bright:
            pwm.set_pwm(x.greenpin, 0, x.greenpwm - 100*i)
        else:
          pwm.set_pwm(x.greenpin, 0, int(ncolors[y].greenpwm*self.bright))
        if 100*i <= x.bluediff:
          if x.bluepwm < ncolors[y].bluepwm*self.bright:
            pwm.set_pwm(x.bluepin, 0, x.bluepwm + 100*i)
          elif x.bluepwm > ncolors[y].bluepwm*self.bright:
            pwm.set_pwm(x.bluepin, 0, x.bluepwm - 100*i)
        else:
          pwm.set_pwm(x.bluepin, 0, int(ncolors[y].bluepwm*self.bright))
        y += 1
  ...
  def shift(self, color): # color must be a color primary key or string hexcode "#AA33FF"
    ...
    for i in range(int(maxdiff/100)):
      for x in t:
        if 100*i <= x.reddiff:
          if x.redpwm < ncolor.redpwm*self.bright:
            pwm.set_pwm(x.redpin, 0, x.redpwm + 100*i)
          elif x.redpwm > ncolor.redpwm*self.bright:
            pwm.set_pwm(x.redpin, 0, x.redpwm - 100*i)
        else:
          pwm.set_pwm(x.redpin, 0, int(ncolor.redpwm*self.bright))
        if 100*i <= x.greendiff:
          if x.greenpwm < ncolor.greenpwm*self.bright:
            pwm.set_pwm(x.greenpin, 0, x.greenpwm + 100*i)
          elif x.greenpwm > ncolor.greenpwm*self.bright:
            pwm.set_pwm(x.greenpin, 0, x.greenpwm - 100*i)
        else:
          pwm.set_pwm(x.greenpin, 0, int(ncolor.greenpwm*self.bright))
        if 100*i <= x.bluediff:
          if x.bluepwm < ncolor.bluepwm*self.bright:
            pwm.set_pwm(x.bluepin, 0, x.bluepwm + 100*i)
          elif x.bluepwm > ncolor.bluepwm*self.bright:
            pwm.set_pwm(x.bluepin, 0, x.bluepwm - 100*i)
        else:
          pwm.set_pwm(x.bluepin, 0, int(ncolor.bluepwm*self.bright))
    for x in t:
